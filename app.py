...
def _is_gold_open() -> bool:
    ...
    if dow == 6 and hour < 18: return False
    if dow == 4 and hour >= 17: return False
    return True
...
if "feature_engine"     not in st.session_state: st.session_state.feature_engine     = None
if "hmm_engine"         not in st.session_state: st.session_state.hmm_engine         = None
if "bayesian_model"     not in st.session_state: st.session_state.bayesian_model     = None
if "insider_engine"     not in st.session_state: st.session_state.insider_engine     = None
if "portfolio_optimizer"not in st.session_state: st.session_state.portfolio_optimizer= None
if "risk_engine"        not in st.session_state: st.session_state.risk_engine        = None
if "loaded"             not in st.session_state: st.session_state.loaded             = False
if "cmd_history"        not in st.session_state: st.session_state.cmd_history        = []
if "signal_result"      not in st.session_state: st.session_state.signal_result      = {}
if "risk_metrics"       not in st.session_state: st.session_state.risk_metrics       = {}
...
if "bs_metrics"         not in st.session_state: st.session_state.bs_metrics         = {}
...
def initialize_engines():
    ...
    if result:
        ...
        # ── Pre-calcular métricas BS al inicializar ────────────────────────────
        try:
            ...
            if spot == spot and spot > 0:
                ...
                bs  = BSEngine()
                bs.run(spot_price=spot, returns_series=de.get_xau_returns())
                st.session_state.bs_metrics = bs.metrics
        except Exception:
            pass
...
def _live_ticker_fragment():
    ...
    refresh_info = (
        f"AUTO ↺ <span style='color:#ffd700'>{remaining}s</span>" if auto_on and market_open
        else "AUTO — awaiting open" if auto_on
        else "MANUAL MODE"
    )
    ...
    # ── IV/HV badge en ticker ─────────────────────────────────────────────────
    bsm = st.session_state.get("bs_metrics", {})
    iv_hv_ratio = bsm.get("IV_HV_ratio", float("nan"))
    if iv_hv_ratio == iv_hv_ratio:
        sig = bsm.get("sizing_signal", "NORMAL")
        sig_color = "#ff4444" if sig == "REDUCE_SIZE" else "#ffd700" if sig == "EXPAND_SIZE" else "#00ff41"
        iv_badge = (
            f"&nbsp;|&nbsp; <span style='color:{sig_color};font-size:9px;'>"
            f"IV/HV {iv_hv_ratio:.2f} [{sig}]</span>"
        )
    else:
        iv_badge = ""
    ...
    <div style="background:#000;border:1px solid #00ff41;border-radius:2px;padding:10px 20px;
      ...
      <div style="margin-left:auto;text-align:right;">
        ...
        <div style="color:#ffd700;font-size:10px;margin-top:2px;">{live_src}{age_html}{iv_badge} &nbsp;|&nbsp; {tf_label}</div>
    ...
    # ── /bs — Black-Scholes engine ────────────────────────────────────────────
        ...
        try:
            ...
            if spot != spot or spot <= 0:
                return "[ERROR] — Precio spot no disponible. Verifica WebSocket."
            bs      = BSEngine()
            returns = de.get_xau_returns() if de else None
            bs.run(spot_price=spot, returns_series=returns)
            st.session_state.bs_metrics = bs.metrics
            return bs.summary()
        except ImportError:
            return "[ERROR] — BSEngine no encontrado. Verifica engines/bs_engine.py."
        except Exception as e:
            return f"[ERROR] — BSEngine: {e}"
        ...
        lines = [
            ...
            "  /bs            → Black-Scholes IV/HV + stops dinámicos",
...
with ctrl_col1:
    ...
    interval_opt = st.selectbox("interval", ["15m", "1h", "4h", "1d"],
        index=["15m", "1h", "4h", "1d"].index(st.session_state.interval),
        label_visibility="collapsed", key="interval_select")
...
with ctrl_col2:
    ...
    if cur_period not in period_choices:
        cur_period = period_choices[0]; st.session_state.period = cur_period
    period_opt = st.selectbox("period", period_choices,
...
if not st.session_state.loaded:
    ...
    if st.session_state.loaded:
        ...
        st.error(
            ...
            "The live gold price (gold-api.com) still works. Click **INITIALIZE** to retry.",
            icon="⚠",
        )
        st.stop()
...
def _insider_live_panel():
    ...
    if txns is not None and not txns.empty:
        ie.data_source = src
        ie.load(txns); ie.build_score_series(); ie.compute_momentum(); ie.detect_clusters()
    else:
        ie.data_source = "LIVE DATA UNAVAILABLE (SEC EDGAR unreachable)"
        ie.load(None); ie.build_score_series(); ie.compute_momentum()
    ...
    st.markdown(
        ...
        unsafe_allow_html=True)
    st.markdown(f'<div style="color:#444;font-size:9px;margin-bottom:4px;">{src}</div>', unsafe_allow_html=True)
    st.markdown(f'<div style="color:#003300;font-size:8px;margin-bottom:6px;">{t("last_fetch")}: {fetch_ts} · {t("auto_refresh_lbl")}</div>', unsafe_allow_html=True)

    if not ins_sum.get("data_available", True):
        st.markdown(
            "<div style='background:#1a0000;border:1px solid #ff4444;border-radius:4px;"
            "padding:8px 14px;font-family:monospace;font-size:10px;color:#ff4444;margin-bottom:8px;'>"
            "⚠ LIVE DATA UNAVAILABLE — SEC EDGAR (Form 4) currently unreachable.</div>",
            unsafe_allow_html=True)
    ...
    if not ie.score_series.empty:
        fig_ins = _go.Figure()
        fig_ins.add_trace(_go.Scatter(x=ie.score_series.index, y=ie.score_series.values,
        ...
        fig_ins.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
    ...
    if not recent_txns.empty:
        st.markdown(f'<div style="color:#00aa33;font-size:9px;letter-spacing:2px;margin-top:4px;border-top:1px solid #003300;padding-top:6px;">{t("recent_form4")}</div>', unsafe_allow_html=True)
        rows_html = ""
        for _, row in recent_txns.iterrows():
            ttype  = row.get("type", "—"); tcol = "#00ff41" if ttype == "BUY" else "#ff4444"
            ticker = row.get("ticker", "—"); ins = str(row.get("insider", "—"))[:20]
            role   = str(row.get("role", "—"))[:12]; val = row.get("value", 0)
            ...
            rows_html += (
                ...
                f'<td style="color:#ffd700;padding:3px 5px;">{ts_s}</td>'
        ...
        st.markdown(
            ...
            '<th style="padding:3px 5px;text-align:left;">DATE</th><th>TKR</th>'
            '<th>INSIDER</th><th>ROLE</th><th>TYPE</th><th style="text-align:right;">VALUE</th>'
            f'</tr></thead><tbody>{rows_html}</tbody></table>', unsafe_allow_html=True)
    else:
        st.markdown('<div style="color:#444;font-size:9px;margin-top:8px;">No Form 4 data available.</div>', unsafe_allow_html=True)


# ─── CMD LIVE OUTPUT FRAGMENT ─────────────────────────────────────────────────
_CMD_REFRESH = {
    "/signal": 30, "/kalman": 30, "/hmm": 60, "/cointegration": 60,
    "/insider": 90, "/markowitz": 120, "/risk": 60, "/bs": 60,
    "/heatmap": None, "/gc3d": None, "/help": None,
}
...
def _cmd_live_output():
    ...
    if not active:
        st.markdown('<div class="cmd-output"><span style="color:#333;">▷  Select a command above or type one below</span></div>', unsafe_allow_html=True)
        return
    ...
    st.markdown(
        f"<div style='display:flex;justify-content:space-between;font-family:monospace;font-size:10px;"
        f"color:#444;border-top:1px solid #0a2a0a;border-bottom:1px solid #0a2a0a;padding:3px 6px;margin-bottom:6px;'>"
        f"<span style='color:#00ff41;letter-spacing:1px;'>▶ {active}</span>"
        f"<span style='color:#335533;'>{lbl}</span></div>", unsafe_allow_html=True)
...
# ─── MAIN TABS ────────────────────────────────────────────────────────────────
(tab_quant, tab_cmd, tab_kalman, tab_hmm, tab_bs,
 tab_heat, tab_gc3d, tab_risk, tab_portfolio,
 tab_map, tab_vwap, tab_anomaly, tab_volprofile) = st.tabs([
    t("tab_quant"), t("tab_cmd"), t("tab_kalman"), t("tab_hmm"),
    "BS/VOL",
    t("tab_heat"), t("tab_gc3d"), t("tab_risk"), t("tab_portfolio"),
    t("tab_map"), t("tab_vwap"), t("tab_anomaly"), t("tab_volprofile"),
])
    ...
    if not st.session_state.loaded:
        ...
        with col_sig:
            ...
            fig_gauge = go.Figure(go.Indicator(
                mode="gauge+number", value=p_long * 100,
                title={"text": "P(LONG) %", "font": {"color": "#00ff41", "family": "monospace", "size": 12}},
                gauge={"axis": {"range": [0, 100], "tickcolor": "#00ff41"}, "bar": {"color": "#00ff41"},
                       "bgcolor": "#000000", "bordercolor": "#00ff41",
                       "steps": [{"range": [0, 40], "color": "#1a0000"}, {"range": [40, 60], "color": "#1a1a00"}, {"range": [60, 100], "color": "#001a00"}],
                       "threshold": {"line": {"color": "#ffd700", "width": 2}, "thickness": 0.75, "value": 50}},
                number={"font": {"color": "#00ff41", "family": "monospace"}},
            ))
            fig_gauge.update_layout(paper_bgcolor="#000000", font_color="#00ff41", height=200, margin=dict(l=20, r=20, t=30, b=10))
            st.plotly_chart(fig_gauge, width="stretch")

        with col_hmm:
            st.markdown(f'<div class="section-header">{t("sh_hmm_detector")}</div>', unsafe_allow_html=True)
            if hmm.fitted:
                state = hmm.current_state(); probs = hmm.current_probs()
                ...
                fig_bar = go.Figure(go.Bar(x=vals, y=cats, orientation="h",
                    marker=dict(color=colors, line=dict(color="#000000", width=1)),
                    text=[f"{v:.3f}" for v in vals], textposition="outside",
                    textfont=dict(color="#00ff41", family="monospace", size=11)))
                fig_bar.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
                    xaxis=dict(range=[0, 1], color="#00ff41", gridcolor="#0a2a0a"),
                    yaxis=dict(color="#00ff41"), font=dict(color="#00ff41", family="monospace"),
                    height=220, margin=dict(l=10, r=60, t=10, b=20))
        ...
        with r3c3:
            ...
            for feat, val in list(latest.items())[:8]:
                color = "#00ff41" if (not np.isnan(val) and val >= 0) else "#ff4444"
                st.markdown(
        ...
        fig_price = go.Figure()
        fig_price.add_trace(go.Scatter(x=xau_price.index, y=xau_price.values,
            mode="lines", name="XAUUSD", line=dict(color="#00ff41", width=1.5)))
        if not kalman_trend.empty:
            fig_price.add_trace(go.Scatter(x=kalman_trend.index, y=kalman_trend.values,
                mode="lines", name="Kalman Trend", line=dict(color="#ffd700", width=2, dash="dot")))
        fig_price.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
...
with tab_cmd:
    ...
    if "active_cmd"  not in st.session_state: st.session_state.active_cmd  = None
    if "cmd_output"  not in st.session_state: st.session_state.cmd_output  = ""
    ...
    for cat_label, cat_color, cat_cmds in CMD_PALETTE:
        row_cols = st.columns([0.85] + [1.5] * len(cat_cmds) + [max(0.01, 10 - 0.85 - 1.5 * len(cat_cmds))])
        row_cols[0].markdown(
            f"<div style='font-family:monospace;font-size:9px;color:{cat_color};"
            f"letter-spacing:2px;padding-top:8px;text-align:right;"
            f"border-right:2px solid {cat_color}22;padding-right:6px;'>{cat_label}</div>",
            unsafe_allow_html=True)
        for idx, cmd in enumerate(cat_cmds):
            is_active  = st.session_state.active_cmd == cmd
            btn_style  = "primary" if is_active else "secondary"
            if row_cols[idx + 1].button(cmd, key=f"cmd_btn_{cmd.replace('/', '_')}",
                                        use_container_width=True, type=btn_style):
                _select_cmd(cmd); st.rerun()

    st.markdown("<div style='border-bottom:1px solid #0a2a0a;margin:8px 0 10px;'></div>", unsafe_allow_html=True)

    inp_col, btn_col, clr_col = st.columns([6, 1, 1])
    manual_cmd    = inp_col.text_input("manual_cmd", placeholder="▷  /command  or  /help",
                                       key="cmd_manual_input", label_visibility="collapsed")
    ...
    if exec_clicked and manual_cmd.strip():
        _select_cmd(manual_cmd.strip().lower()); st.rerun()
    if clear_clicked:
        st.session_state.active_cmd = None; st.session_state.cmd_output = ""
        st.session_state.cmd_history = []; st.rerun()
...
with tab_kalman:
    if not st.session_state.loaded:
        ...
        xau_price = de.get_xau_price()
        if xau_price.empty:
            st.warning("⚠ No XAUUSD price data for this interval/period.")
        else:
            ...
            fig_kf = go.Figure()
            fig_kf.add_trace(go.Scatter(x=xau_price.index, y=xau_price.values,
                mode="lines", name="Observed Price", line=dict(color="#004400", width=1)))
            fig_kf.add_trace(go.Scatter(x=custom_trend.index, y=custom_trend.values,
                mode="lines", name="Kalman Trend", line=dict(color="#00ff41", width=2.5)))
            fig_kf.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
                xaxis=dict(color="#00ff41", gridcolor="#0a2a0a"),
            ...
            fig_noise = go.Figure()
            fig_noise.add_trace(go.Scatter(x=custom_noise.index, y=custom_noise.values,
                mode="lines", name="Residual Noise", line=dict(color="#ffd700", width=1),
                fill="tozeroy", fillcolor="rgba(255,215,0,0.05)"))
            fig_noise.add_hline(y=0, line=dict(color="#00ff41", width=1, dash="dash"))
            fig_noise.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
                xaxis=dict(color="#00ff41", gridcolor="#0a2a0a"),
...
with tab_hmm:
    if not st.session_state.loaded:
        ...
        if hmm.fitted:
            from engines.state_space_engine import STATE_LABELS, STATE_COLORS
            state_series = hmm.state_series(); prob_df = hmm.state_prob_df()
            xau_price    = de.get_xau_price()
            fig_regimes  = go.Figure()
            fig_regimes.add_trace(go.Scatter(x=xau_price.index, y=xau_price.values,
                mode="lines", name="XAUUSD", line=dict(color="#002200", width=1)))
            if not state_series.empty:
                price_aligned = xau_price.reindex(state_series.index)
                if price_aligned.isna().all():
                    combined = pd.concat([xau_price.rename("p"), state_series.rename("s")], axis=1).sort_index().ffill().bfill().dropna()
                    price_aligned = combined["p"].reindex(state_series.index).ffill()
                for s_id, s_label in STATE_LABELS.items():
                    mask = state_series == s_id
                    if mask.sum() > 0:
                        fig_regimes.add_trace(go.Scatter(x=state_series[mask].index,
                            y=price_aligned[mask].values, mode="markers", name=s_label,
                            marker=dict(color=STATE_COLORS[s_id], size=5, opacity=0.8)))
            fig_regimes.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
                xaxis=dict(color="#00ff41", gridcolor="#0a2a0a"),
                yaxis=dict(color="#00ff41", gridcolor="#0a2a0a", title="Price"),
                legend=dict(font=dict(color="#00ff41", family="monospace"), bgcolor="#000000"),
                font=dict(color="#00ff41", family="monospace"),
            ...
            if not prob_df.empty:
                ...
                for i, col in enumerate(prob_df.columns):
                    fig_probs.add_trace(go.Scatter(x=prob_df.index, y=prob_df[col].values,
                        mode="lines", name=col, line=dict(color=colors_list[i], width=1.5),
                        fill="tonexty" if i > 0 else "tozeroy",
                        fillcolor="rgba({},{},{},0.12)".format(
                            int(colors_list[i][1:3],16), int(colors_list[i][3:5],16), int(colors_list[i][5:7],16))))
                fig_probs.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
                    xaxis=dict(color="#00ff41", gridcolor="#0a2a0a"),
            ...
            for i, (s_id, s_label) in enumerate(STATE_LABELS.items()):
                p = hmm.current_probs().get(s_label, 0)
                hm_cols[i].markdown(f'<div class="metric-card"><div class="metric-label">{s_label}</div><div class="metric-value" style="color:{STATE_COLORS[s_id]}">{p:.4f}</div></div>', unsafe_allow_html=True)
            tm = hmm.transition_matrix()
            st.markdown(f'<div class="section-header">{t("sh_transition")}</div>', unsafe_allow_html=True)
            fig_tm = px.imshow(tm, x=[STATE_LABELS[i] for i in range(3)], y=[STATE_LABELS[i] for i in range(3)],
                color_continuous_scale=[[0,"#000000"],[0.5,"#003300"],[1,"#00ff41"]], text_auto=".3f")
            fig_tm.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
                font=dict(color="#00ff41", family="monospace"), height=260, margin=dict(l=10,r=10,t=10,b=10))
...
with tab_bs:
    if not st.session_state.loaded:
        ...
        try:
            ...
            from engines.data_engine import fetch_live_price as _flp

            st.markdown('<div class="section-header">BLACK-SCHOLES ENGINE — IV/HV · STOPS DINÁMICOS · SURFACE</div>', unsafe_allow_html=True)

            de_bs   = st.session_state.data_engine
            live_bs = _cached_live_price()
            spot_bs = live_bs.get("last_price", float("nan"))

            if spot_bs != spot_bs or spot_bs <= 0:
                st.warning("⚠ Precio spot no disponible.")
            else:
                bs_obj = BSEngine()
                bs_obj.run(spot_price=spot_bs, returns_series=de_bs.get_xau_returns())
                m = bs_obj.metrics
                st.session_state.bs_metrics = m

                # ── Fila de métricas ──────────────────────────────────────────
                bc1, bc2, bc3, bc4, bc5 = st.columns(5)
                sig_c = {"REDUCE_SIZE": "#ff4444", "NORMAL": "#00ff41", "EXPAND_SIZE": "#ffd700"}.get(m["sizing_signal"], "#00ff41")
                for col, label, val, color in [
                    (bc1, "IV (GVZ)",      f"{m['IV_pct']:.2f}%",       "#ffd700"),
                    (bc2, "HV 20d",        f"{m['HV_pct']:.2f}%",       "#00ff41"),
                    (bc3, "IV/HV ratio",   f"{m['IV_HV_ratio']:.4f}",   sig_c),
                    (bc4, "Exp move 1d",   f"±{m['exp_move_1d_1s']:.2f}","#00aaff"),
                    (bc5, "Sizing signal", m["sizing_signal"],           sig_c),
                ]:
                    col.markdown(f'<div class="metric-card"><div class="metric-label">{label}</div><div class="metric-value" style="font-size:18px;color:{color}">{val}</div></div>', unsafe_allow_html=True)

                st.markdown("<hr>", unsafe_allow_html=True)
                bl, br = st.columns(2)

                with bl:
                    st.markdown('<div class="section-header">STOPS DINÁMICOS (prob. de toque)</div>', unsafe_allow_html=True)
                    stop_data = [
                        ("Long  stop 15%", m["stop_long_15pct"],   spot_bs - m["stop_long_15pct"]),
                        ("Long  stop 25%", m["stop_long_25pct"],   spot_bs - m["stop_long_25pct"]),
                        ("Short stop 15%", m["stop_short_15pct"],  m["stop_short_15pct"] - spot_bs),
                        ("Short stop 25%", m["stop_short_25pct"],  m["stop_short_25pct"] - spot_bs),
                    ]
                    rows = ""
                    for label, level, dist in stop_data:
                        color = "#00ff41" if "Long" in label else "#ff4444"
                        rows += (f'<tr style="border-bottom:1px solid #0a200a;">'
                                 ...
                                 f'<td style="padding:5px 8px;color:#666;font-size:10px;">dist: {dist:,.2f}</td></tr>')
                    st.markdown(f'<table style="width:100%;border-collapse:collapse;font-family:monospace;">'
                                ...
                                f'<th style="text-align:left;font-size:9px;">PRECIO</th>'
                                f'<th style="text-align:left;font-size:9px;">DISTANCIA</th>'
                                f'</tr></thead><tbody>{rows}</tbody></table>', unsafe_allow_html=True)

                    st.markdown('<div class="section-header" style="margin-top:12px;">ATM OPTIONS (30d teórico)</div>', unsafe_allow_html=True)
                    oc1, oc2 = st.columns(2)
                    oc1.markdown(f'<div class="metric-card"><div class="metric-label">ATM Call</div><div class="metric-value" style="color:#00ff41;font-size:16px;">${m["atm_call_price"]:,.2f}</div></div>', unsafe_allow_html=True)
                    oc2.markdown(f'<div class="metric-card"><div class="metric-label">ATM Put</div><div class="metric-value" style="color:#ff4444;font-size:16px;">${m["atm_put_price"]:,.2f}</div></div>', unsafe_allow_html=True)

                with br:
                    st.markdown('<div class="section-header">GREEKS ATM CALL (30d)</div>', unsafe_allow_html=True)
                    ghtml = ""
                    for name, val, desc in [
                        ...
                        ghtml += (f'<tr style="border-bottom:1px solid #0a200a;">'
                                  ...
                                  f'<td style="padding:5px 8px;color:#555;font-size:9px;">{desc}</td></tr>')
                    st.markdown(f'<table style="width:100%;border-collapse:collapse;font-family:monospace;">{ghtml}</table>', unsafe_allow_html=True)

                st.markdown("<hr>", unsafe_allow_html=True)

                # ── Superficie BS ─────────────────────────────────────────────
                st.markdown('<div class="section-header">SUPERFICIE BS — CALL PRICE vs STRIKE vs VENCIMIENTO</div>', unsafe_allow_html=True)
                iv_val  = m["IV"]
                strikes = np.linspace(spot_bs * 0.88, spot_bs * 1.12, 30)
                tenors  = np.array([7, 14, 30, 60, 90]) / 365.0
                Z_call  = np.zeros((len(tenors), len(strikes)))
                for i, T in enumerate(tenors):
                    for j, K in enumerate(strikes):
                        Z_call[i, j] = bs_price(spot_bs, K, T, 0.05, iv_val, "call")
                fig_surf = go.Figure(data=[go.Surface(
                    x=strikes, y=[int(t*365) for t in tenors], z=Z_call,
                    colorscale=[[0,"#001100"],[0.3,"#005500"],[0.6,"#00aa33"],[1.0,"#00ff41"]],
                    showscale=False, opacity=0.9)])
                fig_surf.update_layout(paper_bgcolor="#000000",
                    scene=dict(
                        xaxis=dict(title="Strike",    color="#00ff41", gridcolor="#0a2a0a", backgroundcolor="#000"),
                        yaxis=dict(title="Días",      color="#00ff41", gridcolor="#0a2a0a", backgroundcolor="#000"),
                        zaxis=dict(title="Call Price", color="#00ff41", gridcolor="#0a2a0a", backgroundcolor="#000"),
                        bgcolor="#000000"),
                    height=420, margin=dict(l=0,r=0,t=0,b=0), font=dict(color="#00ff41", family="monospace"))
                st.plotly_chart(fig_surf, width="stretch")

                # ── Curva de prob de toque ────────────────────────────────────
                st.markdown('<div class="section-header">PROBABILIDAD DE TOQUE vs NIVEL (horizonte 30d)</div>', unsafe_allow_html=True)
                barriers_long  = np.linspace(spot_bs * 0.85, spot_bs * 0.999, 60)
                barriers_short = np.linspace(spot_bs * 1.001, spot_bs * 1.15, 60)
                T30 = 30 / 365.0
                probs_long  = [touch_probability(spot_bs, b, T30, iv_val) for b in barriers_long]
                probs_short = [touch_probability(spot_bs, b, T30, iv_val) for b in barriers_short]
                fig_tp = go.Figure()
                fig_tp.add_trace(go.Scatter(x=barriers_long,  y=[p*100 for p in probs_long],
                    mode="lines", name="Long (downside)", line=dict(color="#ff4444", width=2)))
                fig_tp.add_trace(go.Scatter(x=barriers_short, y=[p*100 for p in probs_short],
                    mode="lines", name="Short (upside)",  line=dict(color="#00ff41", width=2)))
                fig_tp.add_hline(y=15, line=dict(color="#ffd700", width=1, dash="dash"),
                    annotation_text="15% stop", annotation_font_color="#ffd700")
                fig_tp.add_hline(y=25, line=dict(color="#ff6600", width=1, dash="dash"),
                    annotation_text="25% stop", annotation_font_color="#ff6600")
                fig_tp.add_vline(x=spot_bs, line=dict(color="#00ff41", width=1, dash="dot"))
                fig_tp.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
                    ...
                    height=280, margin=dict(l=0,r=0,t=10,b=0), font=dict(color="#00ff41", family="monospace"))
                st.plotly_chart(fig_tp, width="stretch")

        except ImportError:
            st.error("⚠ engines/bs_engine.py no encontrado. Agrega el archivo y reinicia.")
        except Exception as e:
            st.error(f"⚠ Error en BS tab: {e}")


# ═══════════════════════════════════════════════════════════════════════════════
# TAB 6 — ORDER FLOW HEATMAP
# ═══════════════════════════════════════════════════════════════════════════════
with tab_heat:
    if not st.session_state.loaded:
        st.info(t("init_first"))
        ...
        with h_col1: n_bins      = st.slider(t("price_bins"), 30, 120, 60, key="hmap_bins")
        with h_col2: hmap_window = st.slider(t("vol_window"), 5, 60, 20, key="hmap_window")
        from visualization.heatmap import build_order_flow_heatmap
        xau_price = de.get_xau_price(); xau_ret = de.get_xau_returns()
        ...
        st.markdown('<div style="color:#007722;font-size:11px;">HIGH DENSITY → Institutional accumulation zones | GOLD LINE → Current price | Gaussian KDE</div>', unsafe_allow_html=True)


# ═══════════════════════════════════════════════════════════════════════════════
# TAB 7 — GC3D ALPHA SURFACE
# ═══════════════════════════════════════════════════════════════════════════════
with tab_gc3d:
    if not st.session_state.loaded:
        st.info(t("init_first"))
    else:
        de = st.session_state.data_engine; fe = st.session_state.feature_engine
        ...
        if not available_features: available_features = list(FEATURE_LABELS.keys())[:1]
        gc_col1, gc_col2 = st.columns(2)
        with gc_col1:
            selected_feat = st.selectbox(t("z_feature"), available_features,
                format_func=lambda x: FEATURE_LABELS.get(x, x), key="gc3d_feat")
        with gc_col2:
            view_type = st.radio(t("view"), ["Scatter 3D", "Multi-Feature Surface"], horizontal=True, key="gc3d_view")
        xau_price = de.get_xau_price()
        fig_gc3d  = build_gc3d(xau_price, fe.features, feature_col=selected_feat) if view_type == "Scatter 3D" else build_volatility_surface(fe.features)
        st.plotly_chart(fig_gc3d, width="stretch")
        st.markdown('<div style="color:#007722;font-size:11px;">Z = f(time, price, feature) | COLOR = feature intensity | GOLD = current position</div>', unsafe_allow_html=True)
...
with tab_risk:
    if not st.session_state.loaded:
        ...
        de = st.session_state.data_engine; re = st.session_state.risk_engine; rm = re.metrics
        ...
        st.markdown("<hr>", unsafe_allow_html=True)
        xau_ret   = de.get_xau_returns(); xau_price = de.get_xau_price()
        ...
        if not dd_series.empty:
            ...
            fig_dd.add_trace(go.Scatter(x=dd_series.index, y=dd_series.values*100,
                mode="lines", name="Drawdown %", line=dict(color="#ff4444", width=1.5),
                fill="tozeroy", fillcolor="rgba(255,68,68,0.08)"))
            fig_dd.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
                xaxis=dict(color="#00ff41", gridcolor="#0a2a0a"),
        ...
        if not rv_series.empty:
            fig_rv = go.Figure()
            fig_rv.add_trace(go.Scatter(x=rv_series.index, y=rv_series.values,
                mode="lines", name="Rolling VaR 95% (20d)", line=dict(color="#ffd700", width=1.5)))
            fig_rv.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
                xaxis=dict(color="#00ff41", gridcolor="#0a2a0a"),
        ...
        st.markdown('<div style="color:#007722;font-size:11px;">VaR = μ − z_α σ | ES = E[loss | loss > VaR] | DD = (peak − current) / peak</div>', unsafe_allow_html=True)
...
with tab_portfolio:
    if not st.session_state.loaded:
        ...
        de = st.session_state.data_engine; po = st.session_state.portfolio_optimizer
        ...
        if st.button("▶ RUN OPTIMIZER", key="run_optimizer"):
            ...
            po2.run(de.returns); st.session_state.portfolio_optimizer = po2; po = po2
        res = po.result
        if res and res.get("weights"):
            ...
            with pw_col1:
                ...
                weights = res["weights"]
                fig_w = go.Figure(go.Bar(x=list(weights.keys()), y=list(weights.values()),
                    marker=dict(color=list(weights.values()),
                        colorscale=[[0,"#001100"],[0.5,"#00aa33"],[1,"#00ff41"]],
                        line=dict(color="#000000",width=1)),
                    text=[f"{v:.4f}" for v in weights.values()], textposition="outside",
                    textfont=dict(color="#00ff41", family="monospace")))
                fig_w.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
                    xaxis=dict(color="#00ff41"), yaxis=dict(color="#00ff41", range=[0, max(weights.values())*1.3]),
                    font=dict(color="#00ff41", family="monospace"), height=280, margin=dict(l=30,r=20,t=10,b=30))
            ...
            with pw_col2:
                ...
                if not frontier.empty:
                    fig_ef = go.Figure()
                    fig_ef.add_trace(go.Scatter(x=frontier["vol"]*100, y=frontier["ret"]*100,
                        mode="lines", name="Efficient Frontier", line=dict(color="#00ff41", width=2)))
                    fig_ef.add_trace(go.Scatter(x=[res["portfolio_vol"]*100], y=[res["expected_return"]*100],
                        mode="markers", name="Optimal Portfolio", marker=dict(color="#ffd700", size=12, symbol="star")))
                    fig_ef.update_layout(paper_bgcolor="#000000", plot_bgcolor="#000000",
                        xaxis=dict(color="#00ff41", gridcolor="#0a2a0a", title="Vol (%)"),
                        yaxis=dict(color="#00ff41", gridcolor="#0a2a0a", title="Return (%)"),
                        legend=dict(font=dict(color="#00ff41", family="monospace"), bgcolor="#000000"),
                        font=dict(color="#00ff41", family="monospace"), height=280, margin=dict(l=50,r=20,t=10,b=40))
...
with tab_map:
    ...
    with map_top:
        ...
        with mcol1:
            map_period = st.selectbox("Period", ["5d","1mo","3mo"], index=0,
                key="map_period", label_visibility="collapsed")
        with mcol2:
            if st.button("⟳ REFRESH MAP", key="map_refresh"):
                _load_map_data.clear(); st.rerun()
        with mcol3:
            st.markdown("<div style='font-family:monospace;font-size:9px;color:#444;padding-top:8px;text-align:right;'>auto-refresh every 5 min</div>", unsafe_allow_html=True)

    with st.spinner("▶ LOADING MARKET MAP…"):
        sector_df, geo_df, news_items, _gold_chg, _gold_px = _load_map_data(map_period)

    _live_px = _prev_close = _xau_ret = None
    if st.session_state.get("loaded"):
        try:
            _price_data = _cached_live_price()
            _live_px    = float(_price_data["last_price"])
            _prev_close = float(_price_data.get("previous_close") or 0) or None
        except Exception: pass
        try: _xau_ret = st.session_state.data_engine.get_xau_returns()
        except Exception: pass

    ae     = AlertEngine()
    alerts = ae.generate(live_price=_live_px, prev_close=_prev_close, returns=_xau_ret,
    ...
    if st.session_state.get("_last_alert_hash") != _alert_hash:
        ...
        for a in alerts[:2]:
            if   a.severity == "HIGH":   st.toast(f"🔴 {a.asset} — {a.message}", icon="🔴")
            elif a.severity == "MEDIUM": st.toast(f"🟡 {a.asset} — {a.message}", icon="🟡")

    render_map_tab(sector_df, geo_df, alerts, news_items, gold_change_pct=_gold_chg, gold_price=_gold_px)


# ═══════════════════════════════════════════════════════════════════════════════
# TAB 11 — VWAP + SD BANDS
# ═══════════════════════════════════════════════════════════════════════════════
@st.cache_data(ttl=300)
def _load_vwap_data(ticker, interval, period):
    from visualization.vwap_module import _download_intraday, compute_vwap
    df = _download_intraday(ticker, interval, period)
    if not df.empty: df = compute_vwap(df)
    return df

with tab_vwap:
    st.markdown('<div class="section-header">VWAP + BANDAS DE DESVIACIÓN ESTÁNDAR — XAUUSD</div>', unsafe_allow_html=True)
    vcol1, vcol2, vcol3 = st.columns([1, 1, 1])
    with vcol1: vwap_ticker   = st.selectbox("Ticker",   ["GC=F","GLD","IAU","XAUUSD=X"],  key="vwap_ticker")
    with vcol2: vwap_interval = st.selectbox("Intervalo",["5m","15m","1h"],  index=0,       key="vwap_interval")
    with vcol3: vwap_period   = st.selectbox("Período",  ["1d","2d","5d"],   index=1,       key="vwap_period")
    vcol_btn1, _ = st.columns([1, 5])
    with vcol_btn1:
        if st.button("↺ REFRESH", key="vwap_refresh"): _load_vwap_data.clear()
    ...
    st.markdown('<div style="color:#444;font-family:monospace;font-size:10px;">VWAP = Σ(TP×Vol)/Σ(Vol) · Bandas = VWAP ± n×σ · Cache 5 min · Fuente: Yahoo Finance</div>', unsafe_allow_html=True)


# ═══════════════════════════════════════════════════════════════════════════════
# TAB 12 — YFINANCE ANOMALY DR
# ═══════════════════════════════════════════════════════════════════════════════
@st.cache_data(ttl=300)
def _load_anomaly_data(ticker, period, window, threshold):
    from visualization.anomaly_module import build_anomaly_chart
    return build_anomaly_chart(ticker, period, window, threshold)

with tab_anomaly:
    st.markdown('<div class="section-header">YFINANCE ANOMALY DR — DETECCIÓN ESTADÍSTICA DE ANOMALÍAS</div>', unsafe_allow_html=True)
    acol1, acol2, acol3, acol4 = st.columns([1, 1, 1, 1])
    with acol1: anom_ticker    = st.selectbox("Ticker",  ["GC=F","GLD","^SPX","^GSPC","XAUUSD=X"], key="anom_ticker")
    with acol2: anom_period    = st.selectbox("Período", ["2d","5d","10d"], index=1,                key="anom_period")
    with acol3: anom_window    = st.slider("Ventana rolling", 5, 50, 20, 5,                         key="anom_window")
    with acol4: anom_threshold = st.slider("Umbral (σ)",     1.0, 4.0, 2.0, 0.25,                  key="anom_thresh")
    acol_btn1, _ = st.columns([1, 5])
    with acol_btn1:
        if st.button("↺ REFRESH", key="anom_refresh"): _load_anomaly_data.clear()
    with st.spinner("▶ LOADING ANOMALY DATA…"):
        fig_anom, anom_log = _load_anomaly_data(anom_ticker, anom_period, anom_window, anom_threshold)
    anom_chart_col, anom_log_col = st.columns([4, 1])
    with anom_chart_col: st.plotly_chart(fig_anom, width="stretch")
    with anom_log_col:
        st.markdown('<div style="font-family:monospace;font-size:11px;color:#00ff41;letter-spacing:1px;padding:4px 0;border-bottom:1px solid #222;">REGISTRO</div>', unsafe_allow_html=True)
        if anom_log:
            for item in anom_log:
                color = "#ff00ff" if item["direction"] == "UP" else "#00eeff"
                arrow = "▲" if item["direction"] == "UP" else "▼"
                st.markdown(f"<div style='font-family:monospace;font-size:9px;border-bottom:1px solid #111;padding:3px 0;'><span style='color:#444;'>{item['timestamp']}</span><br><span style='color:#00ff41;font-weight:bold;'>{item['tf']}</span> <span style='color:{color};'>{arrow} {item['ret']:+.4f}</span></div>", unsafe_allow_html=True)
        else:
            st.markdown('<div style="color:#333;font-family:monospace;font-size:9px;">Sin anomalías</div>', unsafe_allow_html=True)
    st.markdown('<div style="color:#444;font-family:monospace;font-size:10px;">Retorno = pct_change(Close) · Anomalía = |ret| > media ± umbral×σ · ▲ magenta = UP ▼ cyan = DOWN · Cache 5 min</div>', unsafe_allow_html=True)


# ═══════════════════════════════════════════════════════════════════════════════
# TAB 13 — VOLUME PROFILE SESSIONS
# ═══════════════════════════════════════════════════════════════════════════════
@st.cache_data(ttl=300)
def _load_volprofile_data(ticker, interval, period, n_bins):
...
with tab_volprofile:
    st.markdown('<div class="section-header">VOLUME PROFILE SESSIONS — ANÁLISIS DE PERFIL DE VOLUMEN POR SESIÓN</div>', unsafe_allow_html=True)
    vpcol1, vpcol2, vpcol3, vpcol4 = st.columns([1, 1, 1, 1])
    with vpcol1: vp_ticker   = st.selectbox("Ticker",    ["GC=F","GLD","IAU","^GSPC","QQQ"], key="vp_ticker")
    with vpcol2: vp_interval = st.selectbox("Intervalo", ["5m","15m","1h"],  index=1,        key="vp_interval")
    with vpcol3: vp_period   = st.selectbox("Período",   ["2d","5d","10d"],  index=1,        key="vp_period")
    with vpcol4: vp_bins     = st.slider("Bins de precio", 10, 40, 22, 2,                    key="vp_bins")
    vpcol_btn1, _ = st.columns([1, 5])
    with vpcol_btn1:
        if st.button("↺ REFRESH", key="vp_refresh"): _load_volprofile_data.clear()
    ...
    st.markdown('<div style="color:#444;font-family:monospace;font-size:10px;">POC = precio con mayor volumen (naranja) · VAH/VAL = límites del 70% del volumen · Verde = compradores · Rojo = vendedores · Cache 5 min</div>', unsafe_allow_html=True)